...
const Navbar = ({ cartTotalQty }) => {
  ...
  const navigationAnim = useAnimation();
  ...
  const controlNavbar = () => {
    ...
    if (window.scrollY > lastScrollY) {
      navigationAnim.start("initial");

      setShowSearch(false);
    }
    if (window.scrollY < lastScrollY) {
      navigationAnim.start("animate");
    }
        ...
        <AnimatePresence mode="wait">
          {isShow && (
            <motion.header
              ...
              animate={navigationAnim}
              exit={exitAnimNavigation}
            >
              ...
              <nav
                className={`flex relative z-20 bg-[#0D6EFD] ${style.cShadow}`}
              >
                <div className="container relative flex items-center justify-between ">
                  ...
                  <ul
                    className={`${style.list} flex items-center justify-center `}
                  >
                    {/* Search icon */}
                    <li
              ...
              <div
                className={`blury-bg   hidden -z-10 ${
