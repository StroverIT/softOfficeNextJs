...
function Article({ articleLen, articleData }) {
  ...
  const articleCons = articles[articleLen];
  const itemsCons = articleCons?.items;

  const addItem = (e) => {
    const addItemArticle = articles.map((article, index) => {
      if (articleLen == index) {
        article.items.push({ katNomer: "", cena: "", tipove: "" });
      }
      return article;
    });
    setArticles(addItemArticle);
  };
  const removeArticle = (e) => {
    setArticles(
      articles.filter((item, index) => {
        if (index != articleLen) return item;
      })
    );
  };
  ...
  return (
    <div className="w-full px-2 py-5 border rounded-sm border-gray my-9">
